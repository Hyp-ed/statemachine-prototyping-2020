...
void Main::run() {
    ...
    while(sys_.running_){
      ...
      if (invalid_input_){
        ...
        if (hyped::state_table::table[current_state_][current_event_] != State::kGar) {
          state_machine_data_.current_state_ = hyped::state_table::table[current_state_][current_event_];
        }
